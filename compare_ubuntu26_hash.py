...
def get_remote_last_modified(listing_url: str, target_file: str) -> str:
    try:
        with urlopen(listing_url) as response:
            content = response.read().decode("utf-8", errors="replace")
    except HTTPError as exc:
        raise ConnectionError(f"HTTP error retrieving remote listing: {exc.code} {exc.reason}") from exc
    except URLError as exc:
        raise ConnectionError(f"Error retrieving remote listing: {exc.reason}") from exc

    pattern = re.compile(
        rf'<a href="{re.escape(target_file)}">[^<]*</a></td>\s*<td[^>]*>\s*([^<]+?)\s*</td>'
    )
    match = pattern.search(content)
    if not match:
        raise ValueError(f"Listing entry for '{target_file}' not found at {listing_url}.")
...
def main() -> int:
    ...
    parser.add_argument("--listing", default="https://partner-images.canonical.com/hyper-v/desktop/resolute/current/", help="Remote directory listing URL (used to report last-modified date on a hash mismatch).")
    ...
    if local_hash.lower() == remote_hash.lower():
        print("[OK] Newest already exists: the manifest hash matches the latest remote SHA256SUMS.")
        return 0

    print("[WARN] New version exists: manifest hash does not match the latest remote SHA256SUMS.")

    try:
        last_modified = get_remote_last_modified(args.listing, args.file)
        print(f"Remote last modified: {last_modified}")
    except Exception as exc:
        print(f"WARNING: could not retrieve last-modified date: {exc}", file=sys.stderr)
